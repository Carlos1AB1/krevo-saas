...
export function StockPoliciesView() {
  ...
  const filtered = useMemo(() => {
    return products.filter(p => p.name.toLowerCase().includes(q.toLowerCase()) || p.sku.toLowerCase().includes(q.toLowerCase()));
  }, [q]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold tracking-tight leading-none">Políticas de Inventario y Modelos Logísticos</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            <ul className="text-xs space-y-2 mt-2">
                <li className="flex gap-2"><span className="font-semibold text-nuclear w-18 shrink-0">Insumos:</span> Probabilitícos (Modelo Q / P)</li>
                <li className="flex gap-2"><span className="font-semibold text-nuclear w-18 shrink-0">Mat. Prima:</span> Revisión Continua (EOQ)</li>
                <li className="flex gap-2"><span className="font-semibold text-nuclear w-18 shrink-0">Pt. Termin:</span> FEFO / Bajo Pedido</li>
            </ul>
        </div>
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2 font-medium text-sm text-muted-foreground">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border/50">
              {filtered.map((p) => {
                  const isFinished = p.category === "Dulces" || p.category === "Café Molido" || p.category === "Café en Grano";
                  ...
                  return (
                    <tr key={p.sku} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                            <span className="text-xs bg-muted px-2 py-0.5 rounded font-medium">{p.category}</span>
                            <p className="text-[10px] text-muted-foreground mt-1">
                                {isFinished ? "100% FEFO" : "Mod. Revisión Q"}
                            </p>
                        </td>
                        <td className="px-4 py-3 text-center">
                            <span className={`inline-flex font-bold items-center justify-center size-6 rounded-full text-xs ${p.abc === 'A' ? 'bg-success/20 text-success' : p.abc === 'B' ? 'bg-warning/20 text-warning' : 'bg-info/20 text-info'}`}>{p.abc}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right font-mono text-xs font-semibold text-nuclear">{isFinished ? 'N/A' : eoq}</td>
                        <td className="px-4 py-3 text-right">
                            <span className="px-2 py-1 bg-surface-2 rounded font-mono font-bold text-xs">{p.rop}</span>
                        </td>
                        <td className="px-4 py-3 text-right">
