            ...
            <div className="rounded-xl border border-destructive/50 bg-destructive/10 p-4 flex items-start gap-3 text-destructive shadow-sm">
              ...
              <div>
                ...
                <p className="text-sm mt-1 opacity-90">
                  La pasarela de pagos reportó un rechazo en el último intento de cobro. Las acciones de escritura (crear, editar, eliminar) han sido suspendidas. Actualice su método de pago inmediatamente para restaurar la operación normal del CEDI.
                </p>
                <Button variant="outline" className="mt-3 bg-background border-destructive text-destructive hover:bg-destructive hover:text-destructive-foreground" size="sm">
          ...
          <div className="grid gap-6 md:grid-cols-3">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold flex items-center gap-2">
                      ...
                      {currentPlan.status === "active" ? (
                        <Badge variant="outline" className="bg-success/10 text-success border-success/20 ml-2">Activo</Badge>
                      ) : (
                        <Badge variant="outline" className="bg-destructive/10 text-destructive border-destructive/20 ml-2">En Mora</Badge>
                      )}
                    </h2>
                    <p className="text-sm text-muted-foreground mt-1">
                      Facturación {currentPlan.period === "mes" ? "mensual" : "anual"}. Próximo pago: {currentPlan.nextBillingDate}
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold font-mono">
                      $ {currentPlan.price.toLocaleString("es-CO")} <span className="text-sm font-normal text-muted-foreground">COP</span>
                    </div>
                  </div>
                </div>

                <div className="space-y-4">
                  <h3 className="text-sm font-medium uppercase tracking-wider text-muted-foreground">Uso de Límites del Plan</h3>
                  
                  <div className="grid gap-4 sm:grid-cols-3 pt-2">
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="font-medium">Usuarios</span>
                        <span className="text-muted-foreground">{currentPlan.limits.users.current} / {currentPlan.limits.users.max}</span>
                      </div>
                      <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-nuclear" 
                          style={{ width: `${(currentPlan.limits.users.current / currentPlan.limits.users.max) * 100}%` }}
                        />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-muted-foreground">{currentPlan.limits.warehouses.current} / {currentPlan.limits.warehouses.max}</span>
                      </div>
                      <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-nuclear" 
                          style={{ width: `${(currentPlan.limits.warehouses.current / currentPlan.limits.warehouses.max) * 100}%` }}
                        />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-muted-foreground">{(currentPlan.limits.transactions.current/1000).toFixed(0)}k / {(currentPlan.limits.transactions.max/1000).toFixed(0)}k</span>
                      </div>
                      <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-nuclear" 
                          style={{ width: `${(currentPlan.limits.transactions.current / currentPlan.limits.transactions.max) * 100}%` }}
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div className="rounded-xl border border-border bg-card shadow-sm p-5 sm:p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="border border-border rounded-lg p-4 opacity-70">
                    ...
                    <p className="text-2xl font-bold font-mono mt-2">$99.000 <span className="text-xs font-normal text-muted-foreground">COP/mes</span></p>
                  ...
                  <div className="border-2 border-nuclear rounded-lg p-4 bg-nuclear/5 relative">
                    <div className="absolute top-0 right-0 bg-nuclear text-background text-[10px] uppercase font-bold px-2 py-0.5 rounded-bl-lg rounded-tr-sm">Actual</div>
                    <h4 className="font-medium text-nuclear">Pro (Recomendado)</h4>
                    <p className="text-2xl font-bold font-mono mt-2">$299.000 <span className="text-xs font-normal text-muted-foreground">COP/mes</span></p>
            ...
            <div className="space-y-6">
              <div className="rounded-xl border border-border bg-card shadow-sm p-5">
                ...
                <Button variant="outline" size="sm" className="w-full mt-4">Actualizar Método (ePayco/Wompi)</Button>
              </div>
                        ...
                        <div className="flex items-center gap-1 justify-end mt-0.5">
                          {inv.status === "paid" ? (
                            <>
                              ...
                              <span className="text-[10px] uppercase text-success font-medium">Pagado</span>
                            </>
                          ) : (
                            <>
                              <AlertTriangle className="size-3 text-warning" />
                              <span className="text-[10px] uppercase text-warning font-medium">Pendiente</span>
                            </>
                          )}
                        </div>
                      </div>
